...
export default function Dashboard() {
  ...
  const [showTargetLib, setShowTargetLib] = useState(false);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'm' && FEATURES.markets) setShowMarkets(p => !p);
  ...
  useEffect(() => {
    ...
    const marketTimer = FEATURES.markets ? setTimeout(() => loadMarkets(), 800) : undefined;

    // Priority 2: Space Weather — feeds the top-bar SOLAR readout, and the
    // markets panel when that module is enabled.
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── Uzma Satria lockup ──
                Replaces the rotating ring assembly and the letter-by-letter
                OSIRIS wordmark. The source JPEG had a flat near-black
                background, which would have shown as a rectangle against this
                radial gradient; public/brand/ holds it keyed to transparency. */}
            <motion.img
              src="/brand/uzma-satria-stacked.png"
              alt="Uzma Satria"
              draggable={false}
              initial={{ opacity: 0, scale: 0.92, filter: 'blur(10px)' }}
              animate={{ opacity: 1, scale: 1, filter: 'blur(0px)' }}
              transition={{ duration: 1.1, delay: 0.35, ease: [0.22, 1, 0.36, 1] }}
              className="relative z-[2] w-[min(58vw,368px)] h-auto mb-10 select-none"
            />
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center w-fit">
          {/* Uzma Satria lockup — replaces the previous eye-of-horus mark and
              OSIRIS wordmark. The source art sits in a mostly-empty 2250²
              canvas; public/brand/ holds it cropped to the artwork so the
              height utilities below actually control the rendered size. */}
          <img
            src="/brand/uzma-satria-horizontal.png"
            alt="Uzma Satria"
            draggable={false}
            className="h-7 md:h-9 w-auto shrink-0 select-none drop-shadow-[0_0_12px_rgba(0,0,0,0.85)]"
          />
        </div>
        <div className="flex items-center gap-3 mt-2 min-w-0 pr-4">
      ...
      {FEATURES.promo && isMobile && !showDirections && !navSession && (
      ...
      {showLayers && !isMobile && <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} theme={osirisTheme} setTheme={setOsirisTheme} capabilities={capabilities} layerOpacity={layerOpacity} setLayerOpacity={setLayerOpacity} onFocus={(t) => setFlyToLocation({ lat: t.lat, lng: t.lng, zoom: t.zoom, ts: Date.now() })} />}
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        ...
        <div className="relative group">
          <button onClick={() => { setShowTargetLib(!showTargetLib); setShowIntel(false); setShowAlerts(false); setShowSpaceCam(false); setShowEntityGraph(false); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${showTargetLib ? 'bg-[#F26722]/20' : 'hover:bg-white/10'}`} title="Target Library — IMINT recognition reference and category reporting forms">
            <Layers3 className={`w-4 h-4 ${showTargetLib ? 'text-[#F26722]' : 'text-white/60'}`} />
          </button>
          <span className="absolute right-11 top-1/2 -translate-y-1/2 px-2 py-1 text-[10px] font-mono tracking-wider text-white/80 bg-black/80 backdrop-blur-sm rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">TARGETS</span>
          <AnimatePresence>
            {showTargetLib && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="absolute right-12 top-1/2 -translate-y-1/2 w-[420px]">
                <TargetLibraryPanel />
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className="mobile-nav">
            <div className="glass-panel mobile-nav-inner">
              {[
                ...
                ...(FEATURES.markets ? [{ id: 'markets' as const, icon: BarChart3, label: 'MARKETS' }] : []),
